...
export default function SettingItem({ Icon, title, regular, pressHandler, style }) {
    ...
    return (
        <TouchableOpacity onPress={pressHandler} style={[globalStyles.parentFlexBox, { marginVertical: 5, width: "100%", justifyContent: "space-between", flexDirection: language === 'en' ? "row" : "row-reverse" }]}>
            <View style={[globalStyles.parentFlexBox, { flexDirection: language === 'en' ? "row" : "row-reverse" }]}>
                ...
                <Text style={[globalStyles.regular, { marginHorizontal: Margin.m_base, }, style]} >{title}</Text>
            </View>
