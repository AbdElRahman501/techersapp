import { Keyboard, TouchableWithoutFeedback, ScrollView, View, SafeAreaView, Alert } from 'react-native'
...
export default function VerificationCodeScreen({ route }) {
    ...
    const handleOTPComplete = otp => {
        if (code === otp) {
            Alert.alert('Confirm', 'Your phone number has been verified');
        }
                ...
                <ScrollView style={{ flex: 1 }}>
                    <View style={[globalStyles.container, { minHeight: heightPercentage(80) }]} >
                        <View style={{ width: "100%" }}>
                            ...
                            {phoneNumber && <CustomText style={[globalStyles.regular, { marginBottom: Margin.m_base, color: Color.gray_200, textAlign: "center", }]}>{t("enter-verification-code-description", { number: formatPhoneNumber(phoneNumber) })}</CustomText>}
